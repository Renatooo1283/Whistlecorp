...
// Tres escenas narrativas con ventanas de visibilidad sobre el progreso del
// scroll dentro del hero sticky (0..1). Las pausas entre rangos permiten que
// la animación de frames se vea sin texto encima.
//   • 0.00–0.18 → Hero (texto + CTAs)
//   • 0.18–0.42 → animación pura (sin overlay)
//   • 0.42–0.68 → Propuesta de valor
//   • 0.68–0.78 → animación pura (sin overlay)
//   • 0.78–1.00 → Cierre, logo congelado + CTA
const storyScenes: ScrollStoryScene[] = [
  ...
  // ESCENA 3 — Cierre, logo completo congelado + CTA centrado
  {
    start: 0.78,
    end: 1.0,
    step: {
...
export default function HomePage() {
  return (
    <>
      {/* ─── HERO: scrollytelling canvas con sticky real ───────────────
        - Sección externa de 360vh: provee el recorrido vertical real.
        - Sticky interno (en el componente) ocupa la pantalla bajo el navbar.
        - El frame mostrado deriva 100% de window.scrollY, así funciona también
          al arrastrar la barra lateral.
        - finalFreezeAt=0.78 → el logo queda congelado durante el último 22%.
        - excludeFrameRange={1,40} elimina la "explosión" inicial. */}
      <ScrollStoryCanvas
          ...
          <div className="grid gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20">
            ...
            <ol className="divide-y divide-[var(--color-border)] border-y border-[var(--color-border)]">
              {problems.items.map((item, i) => (
                ...
                >
                  <span className="font-mono text-sm font-medium text-[#86EFAC]">
        ...
        <Container className="py-20 sm:py-24 lg:py-28">
          ...
          <div className="mt-14 grid gap-6 md:grid-cols-2 lg:gap-8">
            {solutions.items.map((s, i) => (
              <article
                key={s.title}
                className="group relative flex flex-col rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-alt)] p-7 transition-colors hover:border-[var(--color-accent)]/40 sm:p-8"
              >
                <div className="flex items-baseline gap-3">
                  <span className="font-mono text-xs font-medium text-[#86EFAC]">
                    {String(i + 1).padStart(2, "0")}
                  </span>
                ...
                <Link
                  href={s.href}
                  className="mt-6 inline-flex items-center gap-1.5 text-sm font-semibold text-[#86EFAC] hover:text-white"
                >
                  Conocer más
          ...
          <div className="mx-auto max-w-3xl">
            ...
            <div className="mt-10 flex items-center gap-3 border-t border-[var(--color-border)] pt-6">
              ...
              <a
                ...
                className="text-sm font-semibold text-[#86EFAC] hover:text-white"
              >
                {finalCta.email}
              </a>
            </div>
