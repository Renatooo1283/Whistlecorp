...
export function ScrollStoryCanvas({
  ...
  const curatedFrames = useMemo(() => {
    ...
    const excludeStartIdx = excludeFrameRange.start - frameStart; // 0-based
    const excludeEndIdx = excludeFrameRange.end - frameStart;
    for (let i = 0; i < frameCount; i++) {
      if (i >= excludeStartIdx && i <= excludeEndIdx) continue;
      arr.push(i);
    }
  ...
  useEffect(() => {
    ...
    // === Scene transition (eased interpolation in curated space) ===
    const goToScene = (nextScene: number) => {
      ...
      // Preload every curated frame within the segment so the transition
      // is smooth even on cold paint
      ...
      // Choose duration — entering the last scene optionally takes longer so
      // the final logo holds on screen for a beat.
      const duration =
        nextScene === lastScene
...
function SceneOverlay({
  ...
  // card content (eyebrow / title / text / CTA) is properly centered everywhere.
  const textAlignCls =
    align === "center" ? "text-center" : "lg:text-left";

  // Center variant also vertically centers the card both on mobile and desktop.
  const flexAlignCls =
    isHero || align === "center"
      ? "items-center"
      : "items-end lg:items-center";

  // When center-aligned we use a column flex inside the card so the CTA
  // sits perfectly below the title with consistent spacing.
  const ctaContainerCls =
    align === "center"
      ? "mt-6 flex flex-col items-center gap-3 lg:mt-7"
  ...
  // When centered, drop the asymmetric lg:max-w-md so the paragraph stays
  // visually balanced under the centered title; use mx-auto on small screens
  // to keep the line ragged but centered.
