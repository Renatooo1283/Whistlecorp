...
export type ScrollStoryCanvasProps = {
  frameCount: number;
  /** Path prefix, e.g. "/scroll-frames/whistlecorp/frame-" */
  framePrefix: string;
...
export function ScrollStoryCanvas({
  ...
  // Build paths once
  const paths = useMemo(() => {
    const arr: string[] = new Array(frameCount);
    for (let i = 0; i < frameCount; i++) {
      arr[i] = `${framePrefix}${String(i + frameStart).padStart(
        framePadding,
        "0"
      )}${frameSuffix}`;
    }
    return arr;
  }, [frameCount, framePrefix, frameSuffix, framePadding, frameStart]);
  ...
  useEffect(() => {
    ...
    const loadFrame = (i: number, priority: "high" | "low" = "low") => {
      ...
      img.onload = () => {
        ...
        // If this frame is the current target, paint it
    ...
    // === initial preload ===
    // Frame 0 with high priority + first 12 frames
    loadFrame(0, "high");
    for (let i = 1; i < Math.min(12, frameCount); i++) loadFrame(i, "high");

    // Background load of remaining frames, throttled
    (async () => {
