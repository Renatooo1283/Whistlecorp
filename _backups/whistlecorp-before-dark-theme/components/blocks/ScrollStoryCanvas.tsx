...
function SceneOverlay({
  ...
  const ctaContainerCls =
    align === "center"
      ? "mt-7 flex flex-col items-center gap-3 lg:mt-8"
      : isHero
      ? "mt-7 flex flex-wrap items-center gap-4 sm:mt-8"
  ...
  const titleCls = isHero
    ? "mt-5 text-[34px] font-bold leading-[1.06] tracking-tight text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.55)] sm:text-[42px] md:text-[52px] lg:text-[60px] lg:leading-[1.05]"
  ...
  const textCls = isHero
    ? "mt-6 max-w-[680px] text-base leading-relaxed text-white/90 drop-shadow-[0_1px_3px_rgba(0,0,0,0.65)] sm:text-lg"
    : align === "center"
  ...
  // Gradient only behind the hero text. Sweeps left → right so the rest of
  // the animation stays visible. Mobile uses a slightly stronger top→bottom
  // mix so the text below stays readable too.
  const heroBgStyle: React.CSSProperties | undefined = isHero
    ? {
        background:
          "linear-gradient(90deg, rgba(5,15,25,0.82) 0%, rgba(5,15,25,0.55) 35%, rgba(5,15,25,0.12) 70%, rgba(5,15,25,0) 100%), linear-gradient(180deg, rgba(5,15,25,0) 40%, rgba(5,15,25,0.35) 100%)",
      }
    : undefined;
