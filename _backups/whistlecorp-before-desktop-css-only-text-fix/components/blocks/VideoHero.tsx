...
export function VideoHero({
  ...
  return (
    ...
    >
      ...
      {/* Mobile video — native controls, no React/hydration dependency */}
      {!videoError && (
        <video
          poster={poster}
          muted
          ...
          preload="auto"
          aria-hidden="true"
          className="pointer-events-auto absolute inset-0 z-0 block h-full w-full object-cover md:hidden"
        >
          <source src={src} type="video/mp4" />
        </video>
      )}
      ...
      {/* Scene overlays (desktop / md+ only — driven by React) */}
      ...
      {/* Mobile CSS-only scenes — no React state, no hydration dependency */}
      <div aria-hidden className="md:hidden">
        <MobileHeroScene
          className="mobile-hero-scene-1"
      ...
      {/* Fallback play button when autoplay is blocked (desktop only) */}
      {autoplayFailed && !videoError && (
        <button
          type="button"
          onClick={handleManualPlay}
          className="absolute bottom-6 left-1/2 z-30 hidden -translate-x-1/2 items-center gap-2 rounded-full border border-white/30 bg-black/55 px-5 py-2.5 text-xs font-semibold uppercase tracking-[0.18em] text-white backdrop-blur transition-colors hover:bg-black/70 md:inline-flex"
...
// ────────────────────────────────────────────────────────────────────
// Mobile CSS-only scene block (independent of React hydration)
// ────────────────────────────────────────────────────────────────────
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background:
            "linear-gradient(180deg, rgba(5,15,25,0.55) 0%, rgba(5,15,25,0.35) 100%)",
        }}
      >
        <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-[#86EFAC]">
          {eyebrow}
        </p>
        <h2 className="mt-3 text-[22px] font-bold leading-[1.15] text-white drop-shadow-[0_2px_6px_rgba(0,0,0,0.55)] sm:text-[26px]">
          {title}
        </h2>
        {text && (
          <p className="mt-3 text-sm leading-relaxed text-white/90 drop-shadow-[0_1px_3px_rgba(0,0,0,0.65)]">
            {text}
          </p>
        )}
        {cta && (
