...
import { ScrollControlledVideo } from "@/components/blocks/ScrollControlledVideo";
import {
  hero,
  trustStrip,
  editorialQuote,
  ...
  return (
    <>
      ...
      <ScrollControlledVideo
        src="/videos/whistlecorp-scroll.mp4"
        sectionHeight="320vh"
      />
        ...
        <Container className="py-20 sm:py-24 lg:py-28">
          <div className="mx-auto max-w-3xl">
            <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[var(--color-accent-hover)]">
              {editorialQuote.eyebrow}
            </p>
            <p className="mt-6 text-2xl font-semibold leading-[1.25] text-[var(--color-text-strong)] sm:text-3xl lg:text-[2.25rem] lg:leading-[1.2]">
              {editorialQuote.body}
            </p>
            <p className="mt-8 max-w-2xl text-base leading-relaxed text-[var(--color-text)]">
              {editorialQuote.body2}
            </p>
          </div>
        </Container>
      </section>

      {/* ─── Problemas (editorial list, NO cards iguales) ─────────── */}
          ...
          <div className="grid gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20">
            ...
            <ol className="divide-y divide-[var(--color-border)] border-y border-[var(--color-border)]">
              {problems.items.map((item, i) => (
                <li key={item.title} className="grid gap-4 py-7 sm:grid-cols-[auto_1fr] sm:items-baseline sm:gap-8">
      ...
      {/* ─── Servicios (zigzag editorial, sin chips verdes uniformes) */}
      ...
      {/* ─── Automation story (narrative flow) ────────────────────── */}
      <section className="bg-white">
        <Container className="py-20 sm:py-24 lg:py-28">
          <div className="mx-auto max-w-3xl text-center">
            <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[var(--color-accent-hover)]">
              {automationStory.eyebrow}
            </p>
            <h2 className="mt-4 text-3xl font-bold leading-tight text-[var(--color-text-strong)] sm:text-[2.25rem]">
              {automationStory.title}
            </h2>
          </div>

          <ol className="mx-auto mt-12 max-w-2xl space-y-3">
            {automationStory.steps.map((step, i) => (
              <li
                key={step}
                className="flex items-baseline gap-4 border-l-2 border-[var(--color-border)] pl-5 transition-colors hover:border-[var(--color-accent)]"
              >
                <span className="font-mono text-xs font-medium text-[var(--color-text)]">
                  {String(i + 1).padStart(2, "0")}
                </span>
                <p className="py-1 text-base leading-relaxed text-[var(--color-text-strong)] sm:text-lg">
                  {step}
                </p>
              </li>
            ))}
          ...
          <div className="mx-auto mt-10 max-w-2xl rounded-2xl border border-[var(--color-accent)]/25 bg-[var(--color-accent)]/[0.04] p-6 sm:p-8">
            <p className="text-base leading-relaxed text-[var(--color-text-strong)] sm:text-lg">
              <span className="font-semibold text-[var(--color-accent-hover)]">
                Ese mismo flujo
              </span>
              , conectado y automatizado, libera al equipo y deja registro de
              cada paso.
            </p>
            <Link
              href={automationStory.cta.href}
